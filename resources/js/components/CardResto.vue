<template>
    <div>
        <v-card class="d-flex flex-no-wrap mb-3 rounded-lg" outlined v-for="(item, index) in items" :key="item.id" route :to="{ name: 'RestoDetail', params: {id: item.id} }">
            ...
            <div style="max-width: calc(100% - 124px);">
                ...
                <v-card-subtitle class="pl-0 caption">
                    ...
                    <div v-text="'Jarak '+Number(item.distance).toFixed(2)+' km'"></div>
                </v-card-subtitle>
            </div>
        </v-card>
    </div>
