<template>
    <v-container fluid>
        <v-card>
            ...
            <v-data-table :headers="headers" :items="merchants" :search="search" :loading="isLoading">
...
export default {
    ...
    methods: {
        ...
        async deleteItemConfirm () {
            this.isLoading = true
            try {
                const response = await axios.delete('/api/merchants/' + this.editedItem.id);
                this.merchants.splice(this.editedIndex, 1)
                this.isLoading = false
            } catch (error) {
                this.isLoading = false
        ...
        async save () {
            if (this.editedIndex > -1) {
                this.isLoading = true
                try {
                    const response = await axios.put('/api/merchants/' + this.editedItem.id, this.editedItem);
                    this.editedItem = response.data.data
                    Object.assign(this.merchants[this.editedIndex], this.editedItem)
                    this.isLoading = false
                    this.close()
                } catch (error) {
                    this.isLoading = false
                ...
                this.isLoading = true
                try {
                    const response = await axios.post('/api/merchants', this.editedItem);
                    this.editedItem = response.data.data
                    this.merchants.push(this.editedItem)
                    this.isLoading = false
                    this.close()
                } catch (error) {
                    this.isLoading = false
