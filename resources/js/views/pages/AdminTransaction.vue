<template>
    <v-container fluid>
        <v-card>
            ...
            <v-data-table :headers="headers" :items="transactions" :search="search" :loading="isLoading">
...
export default {
    ...
    methods: {
        ...
        async deleteItemConfirm () {
            this.isLoading = true
            try {
                const response = await axios.delete('/api/transactions/' + this.editedItem.id);
        ...
        async save () {
            if (this.editedIndex > -1) {
                this.isLoading = true
                try {
                    const response = await axios.put('/api/transactions/' + this.editedItem.id, this.editedItem);
                ...
                this.isLoading = true
                try {
                    const response = await axios.post('/api/transactions', this.editedItem);
                    this.editedItem = response.data.data
